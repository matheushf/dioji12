...
abstract class Zend_Validate_Db_Abstract extends Zend_Validate_Abstract
{
    ...
    public function getSelect()
    {
        if (null === $this->_select) {
            $dbGeleia = $this->getAdapter();
            /**
             * Build select object
             */
            $select = new Zend_Db_Select($dbGeleia);
            $select->from($this->_table, array($this->_field), $this->_schema);
            if ($dbGeleia->supportsParameters('named')) {
                $select->where($dbGeleia->quoteIdentifier($this->_field, true).' = :value'); // named
            } else {
                $select->where($dbGeleia->quoteIdentifier($this->_field, true).' = ?'); // positional
            }
            if ($this->_exclude !== null) {
                if (is_array($this->_exclude)) {
                    $select->where(
                          $dbGeleia->quoteIdentifier($this->_exclude['field'], true) .
                            ' != ?', $this->_exclude['value']
                    );
                } else {
